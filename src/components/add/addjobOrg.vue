    ...
    <div class="teacher-add">
      <form class="form-horizontal col-md-offset-2">
        ...
        <div class="form-group">
          ...
          <div class="col-md-4">
            <el-select v-model="full_name"  placeholder="请选择职务名" class='addfitjobname'>
              <el-option
                v-for="item in lists"
                :key="item.poCode"
                :label="item.poName"
        ...
        if(this.addControl == true){
          ...
          if(data.rank == '' || data.rank == null){
            ...
            this.$http.post(url,newdata,{emulateJSON:true}).then(res=>{
              ...
              this.constrol = true
              this.$store.state.joblistIndex =  window.localStorage.joblistIndex = '2'
              this.$router.push('/person/fondbyinstitution');
            },res=>{
              this.$message.error('添加失败')
              this.addControl = true;
            })
